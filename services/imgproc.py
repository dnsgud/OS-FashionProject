...
# ==========================================
# 3. 수동 의류 등록 모듈 (user_email 구조로 변경)
# ==========================================
...
def build_manual_cloth_data(user_email, input_data):
...
def insert_manual_cloth_to_db(user_email, input_data):
    """구조화된 수동 입력 데이터를 Supabase DB에 직접 저장"""
    try:
        final_data = build_manual_cloth_data(user_email, input_data)
...
def handle_cloth_registration(register_type, user_email, payload):
    """
    ...
    if register_type == 'photo':
        ...
        return process_user_upload(payload, user_email)
        
    elif register_type == 'manual':
        print("[알고리즘 로그] 직접 등록 방식 선택 -> 수동 DB 저장 파이프라인으로 라우팅")
        return insert_manual_cloth_to_db(user_email, payload)
